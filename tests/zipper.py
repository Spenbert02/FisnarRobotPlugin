...
if __name__ == "__main__":
    ...
    with ZipFile(zip_file_path, "w") as zip_obj:
        for top_folder, sub_folders, files in os.walk(content_folder_path):
            if "virtEnv" not in top_folder and "VirtEnv" not in top_folder and "tests" not in top_folder and "pycache" not in top_folder and ".git" not in top_folder and "plugin_download" not in top_folder:  # disregarding virtual environment folder
